...
# pylint: disable=too-many-instance-attributes,invalid-name, unused-argument, arguments-differ, too-many-locals
...
class ResNetDiscriminator(nn.Module):
    ...
    def __init__(
        self, size=64, nc=3, nfilter=64, nfilter_max=512, res_ratio=0.1, s0=4, **kwargs
    ):
        super().__init__()
        self.s0 = s0
        nf = self.nf = nfilter
...
class Classifier(ResNetDiscriminator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        num_class = kwargs.get("num_class", 4)
        self.fc = nn.Linear(self.fc.in_features, num_class)
...
class ConditionalDiscriminator(nn.Module):
    ...
    def __init__(
        self, size=64, nc=3, nfilter=64, nfilter_max=512, res_ratio=0.1, num_class=None
    ):
        super().__init__()
        s0 = self.s0 = 4
        nf = self.nf = nfilter
        ...
        self.conv_img = nn.Conv2d(nc, 1 * nf, 3, padding=1)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        self.resnet1 = nn.Sequential(*blocks1)
        self.resnet2 = nn.Sequential(*blocks2)
        self.fc = nn.Linear(self.nf0 * s0 * s0, 1)

        self.emb = nn.Linear(num_class, self.nf0)
    ...
    def forward(self, x, label):
        ...
        out_pooled = torch.sum(out, axis=(2, 3))  # Global pooling
        out = out.view(batch_size, self.nf0 * self.s0 * self.s0)

        out = self.fc(out)
        out += torch.sum(out_pooled * self.emb(label), axis=1, keepdims=True)
        return out
